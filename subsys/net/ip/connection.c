		      ...
		      struct net_conn_handle **handle)
{
	...
	/*
	 * Since the net_conn_change_remote() determines if the connection is
	 * in use, must to be called after conn_set_used().
	 */
	ret = net_conn_change_remote(*handle, remote_addr, remote_port);
	if (ret) {
		goto error;
	}
