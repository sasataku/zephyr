		    ...
		    void *user_data)
{
	 ...
	 * user_data using net_conn_change_callback() and 3) update the remote
	 * address and port assuming connect() has been called.
	 *
	 * The callback function passed to net_conn_change_callback() must be
	 * the same function as the one passed to net_conn_register(), not the
	 * callback set for the net context passed by recv_udp.
	 */
	if (context->conn_handler) {
		context->recv_cb = cb;
		...
		ret = net_conn_change_remote(context->conn_handler,
					    context->flags & NET_CONTEXT_REMOTE_ADDR_SET ?
								&context->remote : NULL,
